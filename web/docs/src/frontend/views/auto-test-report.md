...
页面内还共享一套领域状态，可在座舱 / 车控之间切换，并通过 `sessionStorage` 保持同一标签页内的联动一致。
...
- 用例编号和用例名称列各自提供表头关键词筛选，支持模糊匹配并自动回到第一页刷新列表
- 车型执行明细同样支持按用例编号、用例名称表头关键词组合筛选，筛选作用于分页前的完整明细
- 日报页只展示真实上传的结果行，`skip` 仅代表显式上报的跳过结果
