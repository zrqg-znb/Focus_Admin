...
const apis = [
  ...
  },
  {
    consumer: '漏合风险',
    method: 'GET / PUT / POST',
    params: '组织、代码库、PL组、分支、状态、时间范围；手动同步时间窗口',
    path: '/api/code-compliance/missing-merges/records* / /api/code-compliance/missing-merges/pl-dashboard / /api/code-compliance/missing-merges/scan-tasks*',
    purpose: '查询自动检测出的漏合 CR，更新处理状态，查看 PL 组趋势，并手动触发数据湖同步',
    returns: 'PaginatedMissingMergeRecordOut / MissingMergePlDashboardOut / MissingMergeScanTaskOut',
  },
];
</script>
