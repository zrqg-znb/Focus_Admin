...
export const CodeAnalysisEngine = {
  ...
  async analyzeCode(code: string, language: string, promptTemplateId?: string): Promise<CodeAnalysisResult> {
    try {
  ...
  async analyzeRepository(params: RepositoryAnalysisParams): Promise<AuditTask> {
    try {
  ...
  async getRepositories(): Promise<Project[]> {
    ...
    return projects.filter((project) => project.source_type === "repository" && Boolean(project.repository_url));
  },

  async getBranches(projectId: string): Promise<{ branches: string[]; default_branch: string; error?: string }> {
