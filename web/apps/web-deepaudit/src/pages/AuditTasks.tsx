...
export default function AuditTasks() {
  ...
  const canCancelFastTask = hasAccess(DEEPAUDIT_ACTION_CODES.TASKS_CANCEL);
  const canCancelAgentTask = hasAccess(DEEPAUDIT_ACTION_CODES.AGENT_TASKS_CANCEL);
  const canExportReports = hasAccess(DEEPAUDIT_ACTION_CODES.REPORTS_EXPORT);
            ...
            if (updated.status === 'running') {
              ...
              if (lastRecord) {
                if (currentProgress !== lastRecord.progress) {
                  ...
                  const warningOptions: Parameters<typeof toast.warning>[1] = {
                    id: `zombie-${updated.id}`,
                    duration: 10000,
                  };
                  if (canCancelFastTask) {
                    warningOptions.action = {
                      label: '取消任务',
                      onClick: () => handleCancelTask(updated.id),
                    };
                  }
                  toast.warning(`任务 "${updated.project?.name || '未知'}" 可能已停止响应`, warningOptions);
                  taskProgressRef.current.set(updated.id, { progress: currentProgress, time: Date.now() });
                }
              } else {
                taskProgressRef.current.set(updated.id, { progress: currentProgress, time: Date.now() });
        ...
        <>
          {filteredAgentTasks.length > 0 ? (
                ...
                <div key={task.id} className="cyber-card p-6">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    ...
                    <div className="flex gap-3">
                      ...
                      {canExportReports && (task.status === 'completed' || (task.findings_count != null && task.findings_count > 0)) && (
                        <Button
                          size="sm"
            ...
            <div className="cyber-card p-16 text-center relative z-10 border-dashed">
              ...
              {!searchTerm && statusFilter === "all" && canCreateAgentTask && (
        ...
        <>
          {filteredTasks.length > 0 ? (
              ...
              {filteredTasks.map((task) => (
                <div key={task.id} className="cyber-card p-6">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    ...
                    <div className="flex gap-3">
                      {(task.status === 'running' || task.status === 'pending') && canCancelFastTask && (
                        <Button
                          size="sm"
                          className="cyber-btn bg-rose-500/90 border-rose-500/50 text-foreground hover:bg-rose-500 h-9"
                      ...
                      {canExportReports && (task.issues_count > 0 || task.status === 'completed') && (
                        <Button
                          size="sm"
                          className="cyber-btn-outline h-9"
            ...
            <div className="cyber-card p-16 text-center relative z-10 border-dashed">
              ...
              {!searchTerm && statusFilter === "all" && canCreateFastTask && (
