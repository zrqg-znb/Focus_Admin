...
export interface GovernanceProject { id: string; name: string; code: string; repository: string; branch: string; description: string; is_active: boolean }
export interface GovernanceResponsibility { id: string; name: string; code: string; description: string; is_active: boolean; owner?: UserOption; approvers: UserOption[] }
export interface GovernanceLink { id: string; project_id: string; project_name: string; responsibility_id: string; responsibility_name: string; is_active: boolean; remark: string }
export interface Finding { id: string; project_name: string; responsibility_name: string; identity_key: string; issue_key: string; fingerprint: string; rule_id: string; severity: string; shield_status: string; latest_tool_name: string; latest_file_path: string; latest_line: number; latest_message: string; last_seen_at?: string; occurrence_id?: string; file_path?: string; start_line?: number; end_line?: number; message?: string; evidence?: unknown[]; identity?: Record<string, unknown>; raw_finding?: Record<string, unknown>; report_complete?: boolean }
export interface Application { id: string; finding_id: string; project_name: string; responsibility_name: string; applicant?: UserOption; approver?: UserOption; reason: string; status: string; audit_comment: string; severity: string; file_path: string; rule_id: string; message: string; created_at?: string }
export interface Summary { total: number; normal: number; pending: number; shielded: number; pending_applications: number; severity: Record<string, number>; project_rank: { name: string; count: number }[]; responsibility_rank: { name: string; count: number }[]; tool_rank: { name: string; count: number }[]; latest_report?: Record<string, unknown> }
...
export const getTrendApi = (params?: Record<string, unknown>) => requestClient.get<{ date: string; count: number }[]>(`${base}/dashboard/trend`, { params });
