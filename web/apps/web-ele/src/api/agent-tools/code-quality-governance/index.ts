...
export interface WorkbenchSummary {
  ...
  risk_projects: { count: number; name: string }[];
  risk_responsibilities: { count: number; name: string }[];
  scan_exceptions: Record<string, unknown>[];
  my_todos: Application[];
...
export interface Overview {
  ...
  pending_application_count: number;
  severity: Record<string, number>;
  responsibilities?: GovernanceLink[];
  projects?: GovernanceLink[];
  recent_reports: Record<string, unknown>[];
}
export interface MatrixData {
  projects: Pick<GovernanceProject, 'id' | 'name' | 'code'>[];
  responsibilities: Pick<GovernanceResponsibility, 'id' | 'name' | 'code'>[];
  cells: GovernanceLink[];
}
...
export const getProjectOverviewApi = (id: string) =>
  requestClient.get<Overview>(`${base}/projects/${id}/overview`);
export const onboardProjectApi = (id: string, data: unknown) =>
  requestClient.post(`${base}/projects/${id}/onboard`, data);
...
export const getResponsibilityOverviewApi = (id: string) =>
  requestClient.get<Overview>(`${base}/responsibilities/${id}/overview`);
export const addCaretakerApi = (id: string, data: unknown) =>
  requestClient.post(`${base}/responsibilities/${id}/caretakers`, data);
export const removeCaretakerApi = (id: string, userId: string) =>
  requestClient.delete(`${base}/responsibilities/${id}/caretakers/${userId}`);
...
export const getMatrixApi = () =>
  requestClient.get<MatrixData>(`${base}/project-responsibilities/matrix`);
export const batchCreateLinksApi = (data: unknown) =>
  requestClient.post(`${base}/project-responsibilities/batch`, data);
export const uploadReportApi = (data: FormData) =>
  requestClient.post(`${base}/reports/upload`, data);
export const ingestReportApi = (data: unknown) =>
  requestClient.post(`${base}/reports/ingest`, data);
export const listReportsApi = (params?: Record<string, unknown>) =>
  requestClient.get<PageResult<Record<string, unknown>>>(`${base}/reports`, {
    params: pageParams(params),
  });
...
export const getWorkbenchSummaryApi = (params?: Record<string, unknown>) =>
  requestClient.get<WorkbenchSummary>(`${base}/workbench/summary`, { params });
export const getWorkbenchTodosApi = () =>
  requestClient.get<Record<string, unknown>>(`${base}/workbench/todos`);
export const getWorkbenchRiskRankingApi = () =>
  requestClient.get<Record<string, unknown>>(`${base}/workbench/risk-ranking`);
