...
type LoadOrganizations = () => Promise<OrganizationItem[]>;
type LoadRepositories = (
  params: RepositoryListParams,
) => Promise<PaginatedResponse<RepositoryItem>>;
...
const props = withDefaults(
  defineProps<{
    ...
    selectedRepositories?: RepositoryItem[];
    selectedTitle?: string;
  }>(),
  {
    candidateEmptyDescription: '请选择组织或输入关键词',
    height: '640px',
...
const emit = defineEmits<{
  (event: 'update:selectedOrganizationId', value: string): void;
  (event: 'update:selectedRepositories', value: RepositoryItem[]): void;
}>();
...
const organizationLoader = computed(
  () => props.loadOrganizations || listOrganizationsApi,
);
const repositoryLoader = computed(
  () => props.loadRepositories || listRepositoriesApi,
);

const currentOrganizationId = ref(props.selectedOrganizationId || '');
...
async function loadOrganizations() {
  ...
  try {
    organizationTree.value =
      (await organizationLoader.value()) as OrganizationTreeNode[];
  } finally {
...
async function loadRepositories(resetPage = false) {
  ...
  if (!currentOrganizationId.value && !keyword) {
  ...
  try {
    const result = await repositoryLoader.value({
      keyword: keyword || undefined,
      organization_id: currentOrganizationId.value || undefined,
...
function applySelectedMap(next: Record<string, RepositoryItem>) {
  // 统一从 map 推导对外数组，保证跨页选择和右侧清单始终同步。
  selectedMap.value = next;
  emit('update:selectedRepositories', Object.values(next));
}

async function selectOrganization(node: OrganizationTreeNode) {
  currentOrganizationId.value = node.id;
  emit('update:selectedOrganizationId', node.id);
  await loadRepositories(true);
}

function clearOrganization() {
  currentOrganizationId.value = '';
  emit('update:selectedOrganizationId', '');
...
watch(
  ...
  (rows) => {
    const next: Record<string, RepositoryItem> = {};
    rows.forEach((item) => {
      next[item.id] = item;
    });
    selectedMap.value = next;
...
watch(
  ...
  (value) => {
    const next = value || '';
    if (next === currentOrganizationId.value) return;
    currentOrganizationId.value = next;
    loadRepositories(true);
  },
);
...
<template>
  <div class="repository-selector-workbench" :style="{ height }">
    <section class="selector-panel organization-panel">
      ...
      <div class="organization-tree-wrap">
        <ElTree
          ...
          node-key="id"
          :props="treeProps"
          :current-node-key="currentOrganizationId"
    ...
    <section class="selector-panel candidate-panel">
      <div class="candidate-heading">
        <div>
          <div class="candidate-title">{{ candidateTitle }}</div>
          <div class="candidate-hint">{{ candidateHint }}</div>
        </div>
        <ElButton v-if="currentOrganizationId" text @click="clearOrganization">
          全局搜索
        </ElButton>
      </div>
      ...
      <ElScrollbar class="candidate-list">
        ...
        <ElEmpty
          v-else
          :description="
            currentOrganizationId || repositoryKeyword
              ? '暂无匹配代码库'
    ...
    <aside class="selector-panel selected-panel">
      <div class="selector-panel__header">
