...
import {
  createContributionCodeBaselineApi,
  downloadContributionBaselineTemplateApi,
  ...
  importContributionBaselinesApi,
...
const baselineForm = reactive({
  baseline_at: dayjs().format('YYYY-MM-DDTHH:mm:ssZ'),
  baseline_lines: 0,
  branch_id: '',
  remark: '',
  repository_id: '',
...
const metricCards = computed(() => {
  ...
  return [
    { label: '当前存量代码量', tone: 'primary', value: data.stock_lines },
    { label: '已覆盖仓库', value: data.baseline_repository_count },
    { label: '已覆盖分支', value: data.baseline_branch_count },
    { label: '缺失基线', tone: data.missing_baseline_count ? 'danger' : '', value: data.missing_baseline_count },
    { label: '本期新增', value: data.added_lines },
    { label: '本期删除', value: data.removed_lines },
    { label: '本期净增', value: data.net_lines },
    { label: '本期总变更', value: data.changed_lines },
  ];
});
...
function renderCharts() {
  renderTrendChart({
    ...
    legend: { top: 0 },
    series: [
      { data: trendRows.value.map((item) => item.added_lines), name: '新增', smooth: true, type: 'line' },
      { data: trendRows.value.map((item) => item.removed_lines), name: '删除', smooth: true, type: 'line' },
      { data: trendRows.value.map((item) => item.net_lines), name: '净增', smooth: true, type: 'line' },
    ],
  ...
  renderCategoryChart({
    ...
    series: [{ data: categories.map((item) => ({ name: item.category_label, value: item.changed_lines })), radius: ['48%', '72%'], type: 'pie' }],
...
function openBaseline(row?: ContributionRankingItem) {
  if (row) {
    const repo = repositoryOptions.value.find((item) => item.project_id === row.project_id);
    const branch = branchOptions.value.find((item) => item.branch_name === row.branch_name);
    baselineForm.repository_id = repo?.id || '';
    baselineForm.branch_id = branch?.id || '';
    baselineForm.baseline_lines = row.stock_lines || row.baseline_lines || 0;
  }
  baselineVisible.value = true;
}

async function submitBaseline() {
  if (!baselineForm.repository_id || !baselineForm.branch_id) {
    ElMessage.warning('请选择代码库和分支');
    return;
  }
  baselineSubmitting.value = true;
  try {
    await createContributionCodeBaselineApi({ ...baselineForm });
    baselineVisible.value = false;
    ElMessage.success('代码量基线已保存');
    await loadDashboard();
  } finally {
...
async function importBaselineFile(file: File) {
  await importContributionBaselinesApi(file);
  ElMessage.success('基线导入完成');
  await loadDashboard();
  return false;
}

async function downloadBaselineTemplate() {
  const data = await downloadContributionBaselineTemplateApi();
  saveBlob(data, 'code_contribution_baseline_template.xlsx');
}
...
async function downloadExportTask(task?: ContributionExportTask) {
  ...
  saveBlob(data, current.file_name || 'code_volume_dashboard.xlsx');
}
    ...
    <div class="contribution-page" v-loading="loading">
      <section class="toolbar-panel">
        <ElForm class="filter-form" label-position="top">
          ...
          <ElFormItem class="filter-actions" label=" ">
            ...
            <ElButton @click="openBaseline()">维护基线</ElButton>
      ...
      <section class="analysis-grid">
        <div class="panel panel-wide">
          <div class="panel-title">合入代码量趋势</div>
      ...
      <section class="table-grid">
        <div class="panel panel-wide">
          <div class="panel-title">仓库 / 分支代码量</div>
          <ElTable :data="repositoryRanking" border height="360">
            ...
            <ElTableColumn align="right" label="当前存量" prop="stock_lines" width="120" />
            <ElTableColumn align="right" label="本期净增" prop="net_lines" width="110" />
            <ElTableColumn align="right" label="本期总变更" prop="changed_lines" width="120" />
            <ElTableColumn label="基线" width="180">
              <template #default="{ row }">
                <ElTag v-if="row.has_baseline" effect="plain" type="success">{{ formatTime(row.baseline_at) }}</ElTag>
                <ElButton v-else link type="warning" @click="openBaseline(row)">补基线</ElButton>
              </template>
            </ElTableColumn>
          </ElTable>
        </div>
        ...
        <div class="panel">
          <div class="panel-title">PL组合入分布</div>
      ...
      <ElDialog v-model="baselineVisible" title="维护代码量基线" width="640px">
        <ElForm label-width="96px">
          <ElFormItem label="代码库">
            <ElSelect v-model="baselineForm.repository_id" clearable filterable placeholder="选择代码库">
              <ElOption v-for="item in repositoryOptions" :key="item.id" :label="`${item.project_name}（${item.project_id}）`" :value="item.id" />
            </ElSelect>
          </ElFormItem>
          <ElFormItem label="分支">
            <ElSelect v-model="baselineForm.branch_id" clearable filterable placeholder="选择分支">
              <ElOption v-for="item in branchOptions" :key="item.id" :label="item.branch_name" :value="item.id" />
            </ElSelect>
          </ElFormItem>
          <ElFormItem label="基线代码量">
            <ElInputNumber v-model="baselineForm.baseline_lines" :min="0" :precision="0" controls-position="right" />
          </ElFormItem>
          <ElFormItem label="统计时间">
            <ElDatePicker v-model="baselineForm.baseline_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" />
          </ElFormItem>
          <ElFormItem label="备注">
            <ElInput v-model="baselineForm.remark" maxlength="200" show-word-limit type="textarea" />
          </ElFormItem>
          <ElFormItem label="批量导入">
            <ElButton @click="downloadBaselineTemplate">下载模板</ElButton>
            <ElUpload :before-upload="importBaselineFile" :show-file-list="false" accept=".xlsx,.xls">
              <ElButton>导入Excel</ElButton>
            </ElUpload>
          </ElFormItem>
        </ElForm>
        <template #footer>
          <ElButton @click="baselineVisible = false">取消</ElButton>
          <ElButton :loading="baselineSubmitting" type="primary" @click="submitBaseline">保存基线</ElButton>
        </template>
      </ElDialog>
