...
const emit = defineEmits<{
  (e: 'searchProject', name: string): void;
  (e: 'pageChangeProject', page: number): void;
}>();
...
function onProjectSearch() {
  projectPage.value = 1;
  emit('searchProject', projectSearchName.value);
}

function onProjectPageChange(page: number) {
  projectPage.value = page;
  emit('pageChangeProject', page);
}
...
<template>
  <div class="space-y-6">
    ...
    <RequirementWorkspacePanel scope="favorites" />
      ...
      <ElSkeleton :loading="loadingTimelines" animated>
        <template #default>
          <div v-if="projectTimelines.length > 0" class="space-y-8">
            <div
              v-for="project in projectTimelines"
              :key="project.id"
              class="border-b border-gray-100 pb-8 last:border-0 last:pb-0 dark:border-gray-800"
            >
          ...
          <div v-else class="flex flex-col items-center py-16 text-center">
            ...
            <div class="mt-4">
              ...
              <ElButton type="primary" @click="goProjectConfig">
                去配置关注项目
              </ElButton>
            </div>
          </div>
