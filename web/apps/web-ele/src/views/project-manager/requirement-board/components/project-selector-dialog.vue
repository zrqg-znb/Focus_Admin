...
import type { RequirementBoardProjectOption } from '#/api/project-manager/requirement_board';
...
const domainOptions = computed(() => {
  ...
  props.projects.forEach((item) => {
    ...
    seen.add(value);
    result.push({
      label: displayDimensionValue(item.domain),
      value,
    });
  });
...
const typeOptions = computed(() => {
  ...
  props.projects.forEach((item) => {
    ...
    seen.add(value);
    result.push({
      label: displayDimensionValue(item.type),
      value,
    });
  });
  return result.sort((left, right) =>
  ...
  >
    <div class="project-selector">
      ...
      >
        ...
        <ElTableColumn label="团队数" width="100" align="center">
          <template #default="{ row }">
            {{ row.sub_teams?.length || 0 }}
          </template>
        </ElTableColumn>
      </ElTable>
    </div>
