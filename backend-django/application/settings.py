...
# CR 关联 DTS 使用独立的无鉴权数据湖接口；生产环境仅需填写 DTS 状态接口地址。
CODE_COMPLIANCE_DTS_RELATION_API_URL_TEMPLATE = _env_str(
    'CODE_COMPLIANCE_DTS_RELATION_API_URL_TEMPLATE',
    'http://apig.yinwang.com/api/v4/projects/{project_id}/change_requests/{change_request_iid}/issues',
)
CODE_COMPLIANCE_DTS_STATUS_API_URL = _env_str('CODE_COMPLIANCE_DTS_STATUS_API_URL', '')
CODE_COMPLIANCE_DTS_FORCE_MOCK = _env_bool('CODE_COMPLIANCE_DTS_FORCE_MOCK', CODE_COMPLIANCE_CR_FORCE_MOCK)
CODE_COMPLIANCE_DTS_API_TIMEOUT = _env_float('CODE_COMPLIANCE_DTS_API_TIMEOUT', 15)
CODE_COMPLIANCE_DTS_API_VERIFY_SSL = _env_bool('CODE_COMPLIANCE_DTS_API_VERIFY_SSL', True)
CODE_COMPLIANCE_DTS_RELATION_MAX_WORKERS = _env_int('CODE_COMPLIANCE_DTS_RELATION_MAX_WORKERS', 5)
...
SKILL_OPTIMIZER_QUEUE = os.environ.get('SKILL_OPTIMIZER_QUEUE', 'skill_optimizer')
# Agent Tools 的模型服务可能是多人共用的内网部署。短暂过载时在服务层退避重试，
# 使所有后续 Agent 共用相同的恢复策略，而不是让每个工具单独实现一套。
AGENT_TOOLS_MODEL_RECOVERY_TIMEOUT_SECONDS = _env_float('AGENT_TOOLS_MODEL_RECOVERY_TIMEOUT_SECONDS', 300)
AGENT_TOOLS_MODEL_CONNECT_TIMEOUT_SECONDS = _env_float('AGENT_TOOLS_MODEL_CONNECT_TIMEOUT_SECONDS', 10)
AGENT_TOOLS_MODEL_READ_TIMEOUT_SECONDS = _env_float('AGENT_TOOLS_MODEL_READ_TIMEOUT_SECONDS', 120)
AGENT_TOOLS_MODEL_RETRY_INITIAL_DELAY_SECONDS = _env_float('AGENT_TOOLS_MODEL_RETRY_INITIAL_DELAY_SECONDS', 2)
AGENT_TOOLS_MODEL_RETRY_MAX_DELAY_SECONDS = _env_float('AGENT_TOOLS_MODEL_RETRY_MAX_DELAY_SECONDS', 30)
