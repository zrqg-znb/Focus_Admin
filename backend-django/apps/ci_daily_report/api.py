...
def list_projects(request, keyword: str = None):
    ...
    return ProjectConfig.objects.filter(query).order_by('-created_at')
...
def subscribe_project(request, data: SubscribeInputSchema):
    user = request.auth  # Assuming GlobalAuth returns the user or request.user is set
    # Note: Ninja's request.auth usually holds the user if configured that way, 
    # but in Django request.user is also standard. 
    # Let's rely on request.user if using standard Django Auth backend via Bearer
    user = request.user
