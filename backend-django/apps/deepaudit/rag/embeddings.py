...
from apps.deepaudit.config_resolver import get_setting, resolve_embedding_config
...
class OpenAIEmbedding(EmbeddingProvider):
    ...
    def __init__(
        ...
        runtime_config = resolve_embedding_config(
            None,
            provider="openai",
            model=model,
            api_key=api_key,
        ...
        self.base_url = runtime_config.get("base_url") or "https://api.openai.com/v1"
        self.model = runtime_config.get("model") or model
...
class QwenEmbedding(EmbeddingProvider):
    ...
    def __init__(
        ...
        self.base_url = runtime_config.get("base_url") or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        self.model = runtime_config.get("model") or model
        self._dimension = self.MODELS.get(self.model, 1024)
...
class EmbeddingService:
    ...
    def __init__(
        ...
        # 确定提供商（保存原始值用于属性访问）
        self.provider = runtime_config["provider"] or get_setting('EMBEDDING_PROVIDER', 'openai')
        self.model = runtime_config["model"] or get_setting('EMBEDDING_MODEL', 'text-embedding-3-small')
        self.api_key = runtime_config.get("api_key")
        self.base_url = runtime_config.get("base_url")
        self.custom_dimension = runtime_config.get("dimensions")
        ...
        self._provider = self._create_provider(
            ...
            dimension=self.custom_dimension,
        )
