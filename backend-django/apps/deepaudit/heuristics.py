...
DEFAULT_RULE_PATTERNS: tuple[RulePattern, ...] = (
    ...
    RulePattern(
        code='C_EMBEDDED_CONCURRENCY',
        issue_type='embedded_concurrency',
        title='Potential embedded concurrency hazard',
        severity=SEVERITY_HIGH,
        patterns=(
            r'\bISR\b',
            r'\bIRQ\b',
            ...
            r'\bDMA\b',
            r'\bvolatile\b',
            r'\btaskENTER_CRITICAL\b',
            r'\btaskEXIT_CRITICAL\b',
            r'\btaskENTER_CRITICAL_FROM_ISR\b',
            r'\btaskEXIT_CRITICAL_FROM_ISR\b',
            r'\bxQueue\w+\b',
            r'\bxSemaphore\w+\b',
            r'\bxTaskNotify\w+\b',
        ),
        suggestion='Protect shared state with explicit synchronization, keep ISR work minimal, and make cross-context ownership obvious.',
        description='Detected ISR/task/DMA/shared-state markers that require concurrency and context-boundary review.',
    ),
    RulePattern(
        code='C_HW_ACCESS',
        issue_type='hardware_access',
        title='Potential hardware access contract issue',
        severity=SEVERITY_HIGH,
        patterns=(
            r'\breadl\s*\(',
            r'\bwritel\s*\(',
        ...
        suggestion='Serialize register access, add barriers or cache maintenance where needed, and avoid concurrent MMIO writes from multiple contexts.',
        description='Detected MMIO, register and cache-coherency access patterns that should be reviewed for context safety.',
    ),
    RulePattern(
        code='C_FMT',
