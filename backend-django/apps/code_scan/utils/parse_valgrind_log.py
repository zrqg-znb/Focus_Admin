#!/usr/bin/env python3
"""
Valgrind 日志解析脚本（供流水线调用）。

示例：
  python apps/code_scan/utils/parse_valgrind_log.py \
    --input ./artifacts/valgrind.log \
    --output ./artifacts/valgrind_findings.json

  python apps/code_scan/utils/parse_valgrind_log.py \
    --input ./artifacts/valgrind.log \
    --format csv \
    --output ./artifacts/valgrind_findings.csv
"""
...
from apps.code_scan.parsers.valgrind_parser import ValgrindParser
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="解析 valgrind 日志为标准扫描结果")
    parser.add_argument("--input", required=True, help="valgrind 原始日志路径（.log/.txt/.xml/.json）")
    parser.add_argument(
        "--output",
...
def main() -> int:
    ...
    if not input_path.exists():
        print(f"[valgrind-parser] 输入文件不存在: {input_path}", file=sys.stderr)
        return 1
    ...
    print(f"[valgrind-parser] 输入: {input_path}")
    print(f"[valgrind-parser] 输出: {output_path}")
    print(f"[valgrind-parser] 解析结果: {len(findings)} 条")
    return 0
