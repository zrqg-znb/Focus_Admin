...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="流水线: 解析 valgrind 日志并上传解析结果")
    parser.add_argument("--log", required=True, help="valgrind 原始日志路径")
...
def parse_valgrind(log_path: Path, output_path: Path, pretty: bool) -> int:
...
def main() -> int:
    ...
    if not log_path.exists():
        print(f"[pipeline-valgrind] 日志文件不存在: {log_path}", file=sys.stderr)
        return 1

    output_path = build_output_path(log_path, args.output)
    findings_count = parse_valgrind(log_path, output_path, args.pretty)
    ...
    print(f"[pipeline-valgrind] log: {log_path}")
