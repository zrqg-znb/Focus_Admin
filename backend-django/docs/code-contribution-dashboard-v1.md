...
如果开发环境已经产生了旧版 CR/MR 贡献数据，且迁移因 `source_change_id` 统一默认值或旧唯一约束失败，可以只清理贡献统计数据后重新采集，不需要删除组织、代码库和分支主数据。

执行顺序：

1. 备份开发数据库，确认当前不是生产库。
2. 恢复并提交与当前代码版本匹配的 migrations 文件。
3. 先执行 `python manage.py migrate code_compliance`，确保字段和唯一约束完成。
4. 执行 `python manage.py reset_contribution_data --confirm-reset-contribution-data`。若开发环境 `DEBUG=False`，追加 `--allow-non-debug`。
5. 重新执行 CR/MR 贡献采集任务；CR 可按需要回补，MR 从当前上线时间之后开始增量采集。

该命令只清理贡献事实、日聚合、采集任务、导出任务和旧基线，不会清理组织、代码库、分支及绑定关系。
