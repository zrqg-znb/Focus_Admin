...
- 历史页领域问题详情：
  ...
  - 响应按领域分组、按目录汇总，并保留目录下各 task ID 的详情入口。同一目录可在多个领域中分别展示。

## 历史页领域详情 Mock 边界

- V1 在后端根据配置、日期、指标、目录和 task ID 生成稳定 Mock 问题数与详情 URL，用于先完成弹窗与 API 契约。
- 历史页依据当前项目配置判断是否打开领域详情，不保存采集当天的领域目录快照。
- 后续接入数据湖问题详情接口时，只替换领域详情服务层的 Mock 取数，不改变 history 页组件和 API 响应结构。
...
- history 页中开启按领域获取的四个问题数指标可按领域 Tab 查看目录问题数和 task ID 详情入口。
