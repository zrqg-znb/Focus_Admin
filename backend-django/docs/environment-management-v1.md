...
- 无人占用且队列为空时，环境用户申请占用会直接成功。前端在空闲且无队列时只展示“占用”按钮，不展示排队入口。
...
- `/environment-management/admin`：管理端。
  ...
  - 项目、车型、占用人使用关键词模糊搜索；测试设备使用多级级联多选，最终按具体测试设备 ID 查询，多个设备取交集。
...
- `/environment-management/user`：用户端。
  ...
  - 项目、车型、占用人使用关键词模糊搜索；测试设备使用多级级联多选，避免大量设备平铺造成选择困难，多个设备取交集。
